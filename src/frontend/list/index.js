...
function List({ month, year }) {
	...
	const days = allDays
		.map((row, i) => {
			...
			return <ListItem key={`row-${i}`} date={date} events={dayEvents} />;
		})
		.filter((i) => !!i);
			...
			{hasPast &&
				(!showPast ? (
					<p className="wporg-meeting-calendar__list-expand">
						<Button icon={expand} onClick={() => setShowPast(true)}>
							{__('Show past meetings', 'wporg-meeting-calendar')}
						</Button>
					</p>
				) : (
					<p className="wporg-meeting-calendar__list-expand">
						...
						>
							{__('Hide past meetings', 'wporg-meeting-calendar')}
						</Button>
					</p>
				))}
